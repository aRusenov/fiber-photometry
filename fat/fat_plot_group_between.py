...
for event in events:
    ...
    predefined_comparisons = [(0, 1), (1, 2)]
    ...
    # df_results = pd.DataFrame(nac_results_list)
    # df_results.to_excel(join(out_dir, f'fat_stats_{event}.xlsx'), sheet_name=event, index=False)
    ...
    excel_path = join(out_dir, f'fat_stats.xlsx')
    try:
